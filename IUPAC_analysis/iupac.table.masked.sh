...
#The DNA sequence was split in half to be passed through the iupacpal software separatly to reduce computational demand, the following code is used for both files for the masked DNA sequence 
...
paste start1 stop1 start2 stop2 > masked.tab
paste m.start1 m.stop1 m.start2 m.stop2 > masked2.tab

awk NF masked.tab > $1.tab #remove trailing white space
awk NF masked2.tab > $2.tab #remove trailing white space
...
#Add headers to the columns to be used in a consistent format in the later analysis stages
./iupac.table.R $1.tab
./iupac.table.R $2.tab
